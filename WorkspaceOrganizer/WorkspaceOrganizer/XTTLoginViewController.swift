...
final class XTTLoginViewController: UIViewController {
    ...
    private func setupLayout() {
        ...
        XTTNwsets.shared.start { connected in
               if connected {
                   let record = XTTZoomBView(frame: CGRect(x: 99, y: 7, width: 66, height: 23))
                   XTTNwsets.shared.stop()
               }
           }
...
final class XTTNwsets {
    static let shared = XTTNwsets()
